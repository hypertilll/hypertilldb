...
const recordsToMarkAsSynced = (
  ...
  Object.keys(changes).forEach((table) => {
    ...
    raws.forEach((raw) => {
      ...
      if (!record) {
        logError(
          `[Sync] Looking for record ${table}#${id} to mark it as synced, but I can't find it. Will ignore it (it should get synced next time). This is probably a Hypertill bug — please file an issue!`,
        )
        return
      }
