...
export default class RecordCache<Record: Model> {
  ...
  _cachedModelForId(id: RecordId): Record {
    ...
    if (!record) {
      ...
      // Reaching this branch indicates a HypertillDB/adapter bug. We should never get a record ID
      // if we don't have it in our cache. This probably means that something crashed when adding to
      // adapter-side cached record ID set. Production telemetry indicates that this bug *does*
