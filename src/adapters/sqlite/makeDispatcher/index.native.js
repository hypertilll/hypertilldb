...
class SqliteNativeModulesDispatcher implements SqliteDispatcher {
  ...
  constructor(
    ...
    if (process.env.NODE_ENV !== 'production') {
      invariant(
        this._bridge,
        `NativeModules.WMDatabaseBridge is not defined! This means that you haven't properly linked HypertillDB native module. Refer to docs for instructions about installation (and the changelog if this happened after an upgrade).`,
      )
...
class SqliteJsiDispatcher implements SqliteDispatcher {
  ...
  constructor(dbName: string, { usesExclusiveLocking }: SqliteDispatcherOptions): void {
    this._db = global.nativeHypertillCreateAdapter(dbName, usesExclusiveLocking)
...
const initializeJSI = () => {
  if (global.nativeHypertillCreateAdapter) {
    return true
  }
...
export function getDispatcherType(options: SQLiteAdapterOptions): DispatcherType {
  if (options.jsi) {
    ...
    logger.warn(
      `JSI SQLiteAdapter not available… falling back to asynchronous operation. This will happen if you're using remote debugger, and may happen if you forgot to recompile native app after HypertillDB update`,
    )
  }
