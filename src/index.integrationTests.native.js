...
if (openPlayground) {
  ...
  AppRegistry.registerComponent('hypertillTest', () => PlaygroundPlaceholder)
  ...
  const TestRoot = () => {
    ...
    return (
      ...
      >
        <View testID="HypertillTesterContent">
          <Text style={{ paddingTop: 100 }}>Hypertill tester!</Text>
          <Text>Using hermes? {global.HermesInternal ? 'YES' : 'NO'}</Text>
          {status === 'testing' ? (
            <Text testID="HypertillTesterStatus" style={{ fontSize: 30 }}>
              The tests are running. Please remain calm.
            </Text>
          ) : null}
          {status === 'done' ? (
            <Text testID="HypertillTesterStatus" style={{ fontSize: 30, color: 'green' }}>
              Done
            </Text>
          ) : null}
          {status === 'error' ? (
            <Text testID="HypertillTesterStatus" style={{ fontSize: 30, color: 'red' }}>
              Error
            </Text>
          ) : null}
        </View>
  ...
  AppRegistry.registerComponent(
    ...
    Platform.OS === 'windows' ? 'HypertillTester' : 'hypertillTest',
    () => TestRoot,
  )
}
