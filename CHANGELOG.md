...
- [Android] Installation of Android JSI adapter has changed. To migrate, remove `getJSIModulePackage()` override in your `MainApplication.{java,kt}`, and add `new HypertillDBJSIPackage()` to `getPackages()` override instead. See Installation docs for details.
...
Hypertill DB now has _experimental_ support for React Native Windows. See Installation docs for details.

**Introducing Hypertill React**

All React/React Native helpers for Hypertill are now available from a new `@hypertill/db/react` folder:

- `DatabaseProvider`, `useDatabase`, `withDatabase`
- NEW: `withObservables` - `@nozbe/with-observables` as a separate package is deprecated, and is now bundled with Hypertill DB
...
Imports from previous `@hypertill/db/DatabaseProvider` and `@hypertill/db/hooks` folders are deprecated and will be removed in a future version.

**Introducing Hypertill Diagnostics**

All debug/dev/diagnostics tools for Hypertill are now available from a new `@hypertill/db/diagnostics` folder:
...
- `@nozbe/with-observables` is no longer a Hypertill DB dependency. Change your imports to `import { withObservables } from '@hypertill/db/react'`

Changes unlikely to cause issues:

- [iOS] If `import HypertillDB` is used in your Swift app (for Turbo sync), remove it and replace with `#import <HypertillDB/HypertillDB.h>` in the bridging header
...
- Imports from `@hypertill/db/DatabaseProvider` and `@hypertill/db/hooks`. Change to `@hypertill/db/react`
...
This is only a small part of Hypertill DB, yet is responsible for a disproportionate amount of issues
...
help to do this! See: https://github.com/helapoint/hypertill-db/issues/1481

### BREAKING CHANGES

- [iOS] You should remove import of Hypertill DB's `SupportingFiles/Bridging.h` from your app project's `Bridging.h`.
  If this removal causes build issues, please file an issue.
- [iOS] In your Podfile, replace previous Hypertill DB's pod imports with this:
...
- Updated `flow-bin` to 0.200. This shouldn't have an impact on you, but could fix or break Flow if you don't have Hypertill DB set to `[declarations]` mode
...
- [JSI+Swift] If you use `hypertilldbProvideSyncJson()` native iOS API, you might need to add `import Hypertill DB`
...
This is a big release to Hypertill DB with new advanced features, great performance improvements, and important fixes to JSI on Android.
...
- All Hypertill console logs are prepended with a `[Hypertill DB]` tag
...
- withObservables is now a dependency of Hypertill DB for simpler installation and consistent updates. You can (and generally should) delete `@nozbe/with-observables` from your app's package.json
...
- [Android] Autolinking is now supported.
  - If You upgrade to `<= v0.21.0` **AND** are on a version of React Native which supports Autolinking, you will need to remove the config manually linking Hypertill DB.
  - You can resolve this issue by **REMOVING** the lines of config from your project which are _added_ in the `Manual Install ONLY` section of the [Android Install docs](https://github.com/helapoint/hypertill-db/Installation.html#android-react-native).
...
Another Hypertill DB release after just a week? Yup! And it's jam-packed full of features!
...
- [Query] `Q.unsafeSqlExpr()` and `Q.unsafeLokiExpr()` are introduced to allow adding bits of queries
  that are not supported by the Hypertill DB query language without having to use `unsafeFetchRecordsWithSQL()`.
...
- [Sync] Introducing Migration Syncs - this allows fully consistent synchronization when migrating
  ...
  After adopting migration syncs, Hypertill Sync will request from backend all missing information.
...
- Added `experimentalDisableObserveCountThrottling()` to `@hypertill/db/observation/observeCount` that globally disables count observation throttling. We think that throttling on Hypertill DB level is not a good feature and will be removed in a future release - and will be better implemented on app level if necessary
...
This is a major update to Hypertill DB.
...
- Improved debuggability of Hypertill -- shortened Rx stacks and added function names to aid in understanding
...
- [withObservables] Added TypeScript support ([changelog](https://github.com/HelaPoint/withObservables/blob/master/CHANGELOG.md))
- [Electron] avoid `Uncaught ReferenceError: global is not defined` in electron runtime ([#453](https://github.com/helapoint/hypertill-db/issues/453))
...
- [Sync] When setting new values to fields on a record, the field (and record) will no longer be
  ...
  `@hypertill/db/Model/index` and call if with `(false)` to bring the old behavior back, but
...
- [Database] You can now update the random id schema by importing
  `import { setGenerator } from '@hypertill/db/utils/common/randomId'` and then calling `setGenerator(newGenenerator)`.
  This allows Hypertill DB to create specific IDs for example if your backend uses UUIDs.
...
- [iOS] Fix runtime crash when built with Xcode 10.2 (Swift 5 runtime).
  ...
  **⚠️ Note**: You need to upgrade to React Native 0.59.3 for this to work. If you can't upgrade
  ...
  https://github.com/helapoint/hypertill-db/pull/302/commits/aa4e08ad0fa55f434da2a94407c51fc5ff18e506
...
- [Hooks] new `useDatabase` hook for consuming the Database Context:
  ```js
  import { useDatabase } from '@hypertill/db/hooks'
  const Component = () => {
...
- LokiJS is now a dependency of Hypertill DB (although it's only required for use on the web)
...
- **Actions API**.
  ...
  This is necessary for Hypertill Sync, and also to enable greater safety and consistency.
...
- **Hypertill Sync Adapter** (Experimental)

  Added `synchronize()` function that allows you to easily add full synchronization capabilities to
  your Hypertill app. You only need to provide two fetch calls to your remote server that conforms
  to Hypertill synchronization protocol, and all the client-side processing (applying remote changes,
  resolving conflicts, finding local changes, and marking them as synced) is done by Hypertill.
...
- The `@hypertill/db/babel/cjs` / `@hypertill/db/babel/esm` Babel plugin that ships with Hypertill is deprecated and no longer necessary. Delete it from your Babel config as it will be removed in a future update
...
- Changed Flow setup for apps using Hypertill - see docs/Advanced/Flow.md
...
- Fixed running model tests in apps with Hypertill in the loop
- Fixed Flow when using Hypertill in apps

## 0.6.0 - 2018-09-05

Initial release of Hypertill DB
