...
To synchronize, you need to pass `pullChanges` and `pushChanges` _(optional)_ that talk to your backend and are compatible with Hypertill Sync Protocol. The frontend code will look something like this:

```js
import { synchronize } from '@hypertill/db/sync'
...
2. You MUST pass `lastPulledAt`, `schemaVersion`, and `migration` to an endpoint that conforms to Hypertill Sync Protocol
3. You MUST return a promise resolving to an object of this shape (your backend SHOULD return this shape already):
   ...
   ```
4. You MUST NOT store the object returned in `pullChanges()`. If you need to do any processing on it, do it before returning the object. Hypertill treats this object as "consumable" and can mutate it (for performance reasons)

### Implementing `pushChanges()`

Hypertill will call this function with a list of changes that happened locally since the last push so you can post it to your backend.
...
1. You MUST pass `changes` and `lastPulledAt` to a push sync endpoint conforming to Hypertill Sync Protocol
...
6. You MUST NOT mutate or store arguments passed to `pushChanges()`. If you need to do any processing on it, do it before returning the object. Hypertill treats this object as "consumable" and can mutate it (for performance reasons)

## Checking unsynced changes

Hypertill DB has a built in function to check whether there are any unsynced changes. The frontend code will look something like this

```js
import { hasUnsyncedChanges } from '@hypertill/db/sync'
...
1. You MUST NOT connect to backend endpoints you don't control using `synchronize()`. Hypertill DB assumes pullChanges/pushChanges are friendly and correct and does not guarantee secure behavior if data returned is malformed.
...
For Hypertill Sync to maintain consistency after [migrations](../Advanced/Migrations.md), you must support Migration Syncs (introduced in Hypertill DB v0.17). This allows Hypertill to request from backend the tables and columns it needs to have all the data.
...
Hypertill DB v0.23 introduced an advanced optimization called "Turbo Login". Syncing using Turbo is up to 5.3x faster than the traditional method and uses a lot less memory, so it's suitable for even very large syncs. Keep in mind:
...
If you're using pullChanges to send additional data to your app other than Hypertill Sync's `changes` and `timestamp`, you won't be able to process it in pullChanges. However, Hypertill DB can still pass extra keys in sync response back to the app - you can process them using `onDidPullChanges`. This works both with and without turbo mode:
...
There's a way to make Turbo Login even more _turbo_! However, it requires native development skills. You need to develop your own native networking code, so that raw JSON can go straight from your native code to Hypertill DB's native code - skipping JavaScript processing altogether.
...
In native code, perform network request and if successful, extract raw response body data - `NSData *` on iOS, `byte[]` on Android. Avoid extracting the response as a string or parsing the JSON. Then pass it to Hypertill DB's native code:

```java
// On Android (Java):
import com.hypertill.db.jsi.HypertillJSI;

HypertillJSI.provideSyncJson(/* id */ syncId, /* byte[] */ data);
```

```objc
// On iOS (Objective-C):
// (If using Swift, add the import to the bridging header)
#import <HypertillDB/HypertillDB.h>

hypertilldbProvideSyncJson(syncId, data, &error)
```
...
import SyncLogger from '@hypertill/db/sync/SyncLogger'
...
require('@hypertill/db/sync/debugPrintChanges').default(changes, isPush)
```
...
Added in Hypertill DB 0.25, there is an alternative way to synchronize changes with the server called "Replacement Sync". You should only use this as last resort for cases difficult to deal with in an incremental fashion, due to performance implications.
...
- When clearing the database, you need to give up all references to Hypertill objects and stop all observation. Therefore, you need to unmount all UI that touches Hypertill, leading to poor UX. This is not required for Replacement Sync
