...
11. Returned record IDs MUST only contain safe characters
    - Default Hypertill DB IDs conform to `/^[a-zA-Z0-9]{16}$/`
...
12. Changes SHOULD NOT contain collections that are not yet present in the local app (at `schemaVersion`). They will, however, be safely ignored.
    - NOTE: This is true for Hypertill DB v0.17 and above. If you support clients using earlier versions, you MUST NOT return collections not known by them.
...
Note that those are not maintained by Hypertill DB, and we make no endorsements about quality of these projects:

- [How to Build Hypertill DB Sync Backend in Elixir](https://fahri.id/posts/how-to-build-watermelondb-sync-backend-in-elixir/)
- [Firemelon](https://github.com/AliAllaf/firemelon)
- [Laravel Hypertill](https://github.com/nathanheffley/laravel-watermelon)
