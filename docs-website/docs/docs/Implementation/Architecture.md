...
`Database` is the root object of Hypertill. It owns:

- a `DatabaseAdapter`
- a map of `Collection`s

`DatabaseAdapter` connects Hypertill's reactive world to low-level imperative world of databases. See [Adapters](./DatabaseAdapters.md).
...
Hypertill's objects and classes are meant to be as minimal as possible — only manage their own state and be an API for your app. Most logic should be stateless, and implemented as pure functions:
...
Model decorators transform simple class properties into Hypertill-aware record fields.
