...
As mentioned previously, you can't just modify Hypertill DB's database anywhere. All changes must be done within a **Writer**.
...
Hypertill DB is highly asynchronous, which is a BIG challange in terms of achieving consistent data. Read this only if you are curious:
...
Because Hypertill DB is asynchronous, if you make multiple separate queries, normally you have no guarantee that no records were created, updated, or deleted between fetching these queries.
...
➡️ Now that you've mastered all basics of Hypertill, go create some powerful apps — or keep reading [**advanced guides**](./README.md)
