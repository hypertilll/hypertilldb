...
After you [define some Models](./Model.md), it's time to connect Hypertill to your app's interface. We're using React in this guide, however Hypertill DB can be used with any UI framework.
...
However, Hooks are not compatible with Hypertill DB's asynchronous API. You _could_ use alternative open-source Hooks for Rx Observables, however we don't recommend that. They won't work correctly in all cases and won't be as optimized for performance with Hypertill DB as `withObservables`. In the future, once Concurrent React is fully developed and published, Hypertill DB will have official hooks.

**[See discussion about official `useObservables` Hook](https://github.com/HelaPoint/withObservables/issues/16)**
...
1. **Custom Observables**. `withObservables` is a general-purpose HOC for Observables, not just Hypertill. You can create new props from any `Observable`.
...
```js
import { compose } from '@hypertill/db/react'
...
import { withObservables, compose } from '@hypertill/db/react'
...
```jsx
import { withDatabase, compose } from '@hypertill/db/react'
