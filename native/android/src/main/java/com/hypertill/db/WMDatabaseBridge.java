...
import com.hypertill.db.utils.MigrationSet;
import com.hypertill.db.utils.Schema;
...
public class WMDatabaseBridge extends ReactContextBaseJavaModule {
    ...
    public void provideSyncJson(int id, String json, Promise promise) {
        // Note: HypertillJSI is optional on Android, but we don't want users to have to set up
        // yet another NativeModule, so we're using Reflection to access it from here
        try {
            Class<?> clazz = Class.forName("com.hypertill.db.jsi.HypertillJSI");
