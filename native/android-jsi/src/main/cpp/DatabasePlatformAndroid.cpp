...
#define LOG_TAG "hypertilldb.jsi"
#define SQLITE_LOG_TAG "hypertilldb.sqlite"
...
void configureJNI(JNIEnv *env) {
    ...
    if (env->GetJavaVM(&jvm) != JNI_OK) {
        consoleError("Could not initialize HypertillDB JSI - cannot get JavaVM");
        std::abort();
    }
    assert(jvm);
...
std::string resolveDatabasePath(std::string path) {
    ...
    jclass clazz = env->FindClass("com/hypertill/db/jsi/JSIInstaller");
    if (clazz == NULL) {
