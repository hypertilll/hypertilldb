...
// Or create another layer of abstraction for JSI-capable SQlite, but without Hypertill-specific logic?
...
jsi::Object Database::resultDictionary(sqlite3_stmt *statement) {
    ...
    for (int i = 0, len = sqlite3_column_count(statement); i < len; i++) {
        ...
        if (type == SQLITE_INTEGER) {
            ...
            throw jsi::JSError(rt, "Unable to fetch record from database - unknown column type (HypertillDB does not support blobs or custom sqlite types");
        }
    }
...
void Database::rollback() {
    ...
    consoleError("HypertillDB sqlite transaction is being rolled back! This is BAD - it means that there's either a "
                 "HypertillDB bug or a user issue (e.g. no empty disk space) that Hypertill may be unable to recover "
